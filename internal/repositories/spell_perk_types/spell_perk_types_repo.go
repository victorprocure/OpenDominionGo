...
//go:embed sql/insert_spell_perk_type.sql
var insertSpellPerkTypeSQL string

//go:embed sql/get_spell_perk_type_by_key.sql
var getSpellPerkTypeByKeySQL string
...
func NewSpellPerkTypesRepo(db *sql.DB, log *slog.Logger) *Repo { return &Repo{db: db, log: log} }

func (r *Repo) CreateContext(ctx context.Context, tx repositories.DbTx, key string) (int, error) {
	var id int
	if err := tx.QueryRowContext(ctx, insertSpellPerkTypeSQL, key).Scan(&id); err != nil {
		return 0, fmt.Errorf("insert spell_perk_type: %w", err)
	}
	return id, nil
}
...
func (r *Repo) GetByKeyContext(ctx context.Context, tx repositories.DbTx, key string) (*Row, error) {
	var x Row
	if err := tx.QueryRowContext(ctx, getSpellPerkTypeByKeySQL, key).Scan(&x.ID, &x.Key); err != nil {
		if err == sql.ErrNoRows {
			return nil, sql.ErrNoRows
		}
		return nil, fmt.Errorf("get spell_perk_type: %w", err)
	}
	return &x, nil
}

func (r *Repo) ListContext(ctx context.Context, tx repositories.DbTx, limit, offset int) ([]Row, error) {
	rows, err := tx.QueryContext(ctx, listSpellPerkTypesSQL, limit, offset)
	if err != nil {
