...
func NewMessageBoardThreadsRepo(db *sql.DB, log *slog.Logger) *Repo { return &Repo{db: db, log: log} }
...
func (r *Repo) GetByIDContext(ctx context.Context, tx repositories.DbTx, id int) (*Row, error) {
	var t Row
	if err := tx.QueryRowContext(ctx, getThreadByIDSQL, id).
		Scan(&t.ID, &t.CategoryID, &t.UserID, &t.Title, &t.Body, &t.FlaggedForRemoval, &t.FlaggedBy, &t.LastActivity); err != nil {
...
func (r *Repo) ListByCategoryContext(ctx context.Context, tx repositories.DbTx, categoryID int, limit, offset int) ([]Row, error) {
	rows, err := tx.QueryContext(ctx, listThreadsByCategorySQL, categoryID, limit, offset)
	if err != nil {
